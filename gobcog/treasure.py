...
class Treasure:
    ...
    common = {
        "rusty sword": {"slot": ["right"], "att": 1, "cha": -1},
    ...
    rare = {
        ".ceremonial_dagger": {"slot": ["right"], "att": 2, "cha": 2},
    ...
    async def open_chest(ctx, user, chest_type):
        if hasattr(user, "display_name"):
            await ctx.send(
                "{} is opening a treasure chest. What riches lay inside?".format(user.display_name)
            )
        else:
            await ctx.send(
    ...
    def countdown(
        ctx, seconds, title, loop: Optional[asyncio.AbstractEventLoop] = None
    ) -> asyncio.Task:
