...
class Classes:
    ...
    async def forge(ctx, item1, item2):
        ...
        if len(newslot) == 2:
            await ctx.send(
                "Your forging roll was 🎲({}).\nThe device you tinkered will have {}🗡 and {}🗨 and be {}.".format(
                    roll, newatt * 2, newdip * 2, hand
                )
            )
        else:
            await ctx.send(
                "Your forging roll was 🎲({}).\nThe device you tinkered will have {}🗡 and {}🗨 and be {}.".format(
                    roll, newatt, newdip, hand
                )
            )
        await ctx.send(
            "Please respond with a name for your creation within 30s.\n(You will not be able to change it afterwards. 40 characters maximum.)"
        )
        reply = None
        try:
    ...
    async def rage(ctx, users):
        ...
        await ctx.send(
            "**{}** is starting to froth at the mouth...🗯️".format(ctx.author.display_name)
        )
        Adventure.users = users
        return users
    ...
    async def sing(ctx, users):
        ...
        users[str(user)]["class"]["ability"] = True
        await ctx.send(
            "♪♫♬ **{}** is whipping up a performance. ♬♫♪".format(ctx.author.display_name)
        )
        Adventure.users = users
        return users
    ...
    async def pet(ctx, users, flag):
        ...
        if flag == "free":
            if users[str(user)]["class"]["ability"] != False:
                ...
                await ctx.send(
                    "**{}** released their pet into the wild.".format(ctx.author.display_name)
                )
            else:
                ctx.command.reset_cooldown(ctx)
                await ctx.send("You have no pet to release.")
        elif flag == "forage":
            return await Treasure.open_chest(
                ctx, users[str(user)]["class"]["ability"]["pet"]["name"], "pet"
            )
        else:
            if users[str(user)]["class"]["ability"] == False:
                ...
                roll = random.randint(1, 20)
                await ctx.send(
                    "You are tracking a wild **{}** with a roll of 🎲({}).".format(
                        Classes.pets[pet]["name"], roll
                    )
                )
                ...
                if dipl_value > Classes.pets[pet]["cha"]:
                    await ctx.send(
                        "You successfully tamed the {}.".format(Classes.pets[pet]["name"])
                    )
                ...
                ctx.command.reset_cooldown(ctx)
                await ctx.send(
                    "You already have a pet. Try foraging (`{}pet forage`).".format(ctx.prefix)
                )
                return None
