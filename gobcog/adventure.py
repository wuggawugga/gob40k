...
class Adventure:
    ...
    async def simple(ctx, users):
        ...
        if Adventure.timeout != 0:
            return None
        Adventure.challenge = random.choice(
            list(Adventure.monsters.keys())
        )  # if you want the dict with accompanying subkeys use: Adventure.monsters[random.choice(list(Adventure.monsters.keys()))]
        Adventure.attrib = random.choice(list(Adventure.attribs.keys()))
        Adventure.str = (
            Adventure.monsters[Adventure.challenge]["str"] * Adventure.attribs[Adventure.attrib][0]
        )
        Adventure.dipl = (
            Adventure.monsters[Adventure.challenge]["dipl"]
            * Adventure.attribs[Adventure.attrib][1]
        )
    ...
    async def choice(ctx):
        if Adventure.challenge == "Red Dragon":
            ...
            await Adventure.menu(
                ...
                [
                    (
                        "but **a{} {}** is guarding it with{}. \n\nWhat will you do and will other heroes help your cause?\nHeroes have 30s to participate via reaction:"
                    ).format(Adventure.attrib, Adventure.challenge, random.choice(threatee))
                ],
                {
                    "🗡": Adventure.fight,
                    "🗨": Adventure.talk,
    ...
    async def menu(
        ...
        try:
            ...
            return
        await controls[react.emoji](
            ctx, pages, controls, message, page, Adventure.timeout, react.emoji, user
        )
    ...
    async def run(
        ...
        await Adventure.check(
            check_other, "run", ctx, pages, controls, message, page, Adventure.timeout, emoji, user
        )
    ...
    async def check(
        check_lists, call_from, ctx, pages, controls, message, page, timeout, emoji, user
    ):
    ...
    async def result(
        ...
        failed = False
        people = (
            len(Adventure.userslist["fight"])
            + len(Adventure.userslist["talk"])
            + len(Adventure.userslist["pray"])
        )
        ...
        async def handle_fight(fumblelist, critlist, attack):
            ...
            for user in Adventure.userslist["fight"]:
                ...
                if roll == 1:
                    ...
                    if (
                        users[str(member.id)]["class"]["name"] == "Berserker"
        ...
        async def handle_pray(fumblelist, attack, diplomacy):
            ...
            for user in Adventure.userslist["pray"]:
                member = discord.utils.find(lambda m: m.display_name == user, ctx.guild.members)
                if (
                    users[str(member.id)]["class"]["name"] == "Cleric"
                    and users[str(member.id)]["class"]["ability"]
                ):
                    roll = random.randint(1, 20)
                    ...
                    if roll == 1:
                        ...
                        await ctx.send(
                            ...
                            + user
                            + "**"
                            + "'s sermon offended the mighty Herbert. (-{}🗡/-{}🗨)".format(
                                5 * len(Adventure.userslist["fight"]),
                                5 * len(Adventure.userslist["talk"]),
                            )
                        )
                        ...
                        await ctx.send(
                            ...
                            + user
                            + "**"
                            + "'s blessed you all in Herbert's name. (+{}🗡/+{}🗨)".format(
                                1 * len(Adventure.userslist["fight"]),
                                1 * len(Adventure.userslist["talk"]),
                            )
                        )
                        ...
                        await ctx.send(
                            ...
                            + user
                            + "**"
                            + " turned into an avatar of mighty Herbert. (+{}🗡/+{}🗨)".format(
                                10 * len(Adventure.userslist["fight"]),
                                10 * len(Adventure.userslist["talk"]),
                            )
                        )
                else:
                    ...
                    if len(Adventure.userslist["fight"] + Adventure.userslist["talk"]) == 0:
                        await ctx.send(
                            ...
                            + user
                            + "**"
                            + " prayed like a madman but nobody else helped them."
                        )
                    if roll == 4:
                        ...
                        await ctx.send(
                            ...
                            + user
                            + "**"
                            + "'s prayer called upon the mighty Herbert to help you. (+{}🗡/+{}🗨)".format(
                                20 * len(Adventure.userslist["fight"]),
                                20 * len(Adventure.userslist["talk"]),
                            )
                        )
                    else:
                        fumblelist.append(user)
            ...
            for user in Adventure.userslist["talk"]:
                ...
                if roll == 1:
                    await ctx.send(
                        ...
                        + user
                        + "**"
                        + (" accidentally offended the {}.").format(Adventure.challenge)
                    )
                    fumblelist.append(user)
                    if (
                        users[str(member.id)]["class"]["name"] == "Bard"
                        and users[str(member.id)]["class"]["ability"]
                    ):
                        bonus = random.randint(5, 15)
                        attack += roll - bonus + att_value
        ...
        async def handle_basilisk(failed):
            if Adventure.challenge == "Basilisk":
                failed = True
                for user in (
                    Adventure.userslist["fight"]
                    + Adventure.userslist["talk"]
                    + Adventure.userslist["pray"]
        ...
        if isinstance(current_page, discord.Embed):
            ...
            await message.edit(content=current_page)
        if people == 0:
            pages = ["but everyone ran away. You failed."]
            Adventure.participants = (
                Adventure.userslist["fight"]
                + Adventure.userslist["talk"]
                + Adventure.userslist["pray"]
                + Adventure.userslist["run"]
                + fumblelist
            )
            await message.edit(content=pages[0])
            return
        ...
        fighters = " and ".join(
            [", ".join(Adventure.userslist["fight"][:-1]), Adventure.userslist["fight"][-1]]
            if len(Adventure.userslist["fight"]) > 2
            else Adventure.userslist["fight"]
        )
        talkers = " and ".join(
            [", ".join(Adventure.userslist["talk"][:-1]), Adventure.userslist["talk"][-1]]
            if len(Adventure.userslist["talk"]) > 2
            else Adventure.userslist["talk"]
        )
        preachermen = " and ".join(
            [", ".join(Adventure.userslist["pray"][:-1]), Adventure.userslist["pray"][-1]]
            if len(Adventure.userslist["pray"]) > 2
            else Adventure.userslist["pray"]
        )
        ...
        if slain or persuaded and not failed:
            ...
            treasure = [0, 0, 0]
            if (
                CR >= 80 or Adventure.challenge == "Basilisk"
        ...
        if people == 1:
            if slain:
                text = ("**{}** has slain the {} in epic battle!").format(
                    fighters, Adventure.challenge
                )
                text += await Adventure.reward(
                    ctx,
                    Adventure.userslist["fight"] + Adventure.userslist["pray"],
                    amount,
                    (attack / Adventure.str),
                    treasure,
                )

            if persuaded:
                text = ("**{}** almost died in battle,").format(talkers) + (
                    " but confounded the {} in the last second."
                ).format(Adventure.challenge)
                text += await Adventure.reward(
                    ctx,
                    Adventure.userslist["talk"] + Adventure.userslist["pray"],
                    amount,
                    (diplomacy / Adventure.dipl),
                    treasure,
                )

            if not slain and not persuaded:
                options = [
                    "No amount of diplomacy or valiant fighting could save you.",
                    "This challenge was too much for one hero.",
                    "You tried your best, but couldn't succeed alone.",
                ]
                text = random.choice(options)
        else:
            if slain and persuaded:
                if len(Adventure.userslist["pray"]) > 0:
                    text = ("**{}** slayed the {} in battle,").format(
                        fighters, Adventure.challenge
                    ) + (
                        " while **{}** distracted with flattery and **{}** aided in Herbert's name."
                    ).format(
                        talkers, preachermen
                    )
                else:
                    text = ("**{}** slayed the {} in battle,").format(
                        fighters, Adventure.challenge
                    ) + (" while **{}** distracted with insults.").format(talkers)
                text += await Adventure.reward(
                    ctx,
                    Adventure.userslist["fight"]
                    + Adventure.userslist["talk"]
                    + Adventure.userslist["pray"],
                    amount,
                    (attack / Adventure.str + diplomacy / Adventure.dipl),
                    treasure,
                )

            if not slain and persuaded:
                if len(Adventure.userslist["pray"]) > 0:
                    text = ("**{}** talked the {} down with **{}'s** blessing.").format(
                        talkers, Adventure.challenge, preachermen
                    )
                else:
            ...
            if slain and not persuaded:
                if len(Adventure.userslist["pray"]) > 0:
                    text = (
                        "**{}** killed the {} in a most heroic battle with a little help from **{}**."
                    ).format(fighters, Adventure.challenge, preachermen)
                else:
                    text = ("**{}** killed the {} in an epic fight.").format(
                        fighters, Adventure.challenge
                    )
                text += await Adventure.reward(
                    ctx,
            ...
            if not slain and not persuaded:
                options = [
                    "No amount of diplomacy or valiant fighting could save you.",
                    "This challenge was too much for this group.",
                    "You tried your best, but succumbed to overwhelming forces in the end.",
                ]
                text = random.choice(options)
    ...
    async def reward(ctx, list, amount, modif, special):
        ...
        for user in list:
            ...
            if (
                roll == 5
                and Adventure.users[str(member.id)]["class"]["name"] == "Ranger"
                and not isinstance(Adventure.users[str(member.id)]["class"]["ability"], bool)
            ):
                Adventure.rewards[user]["xp"] = int(
                    xp * Adventure.users[str(member.id)]["class"]["ability"]["pet"]["bonus"]
                )
                Adventure.rewards[user]["cp"] = int(
                    cp * Adventure.users[str(member.id)]["class"]["ability"]["pet"]["bonus"]
                )
                percent = round(
        ...
        if special != False and sum(special) == 1:
            ...
            phrase += "\nYou have been awarded {} xp and found {} {}. You also secured **a{} treasure chest**!".format(
                xp, cp, currency_name, chest_type
            )
        elif special != False and sum(special) > 1:
            phrase += "\nYou have been awarded {} xp and found {} {}. You also secured **several treasure chests**!".format(
                xp, cp, currency_name
            )
        else:
            phrase += "\nYou have been awarded {} xp and found {} {}.".format(
                xp, cp, currency_name
            )
        return phrase

    def countdown(
        ctx, seconds=None, title="Remaining: ", loop: Optional[asyncio.AbstractEventLoop] = None
    ) -> asyncio.Task:
        async def countdown():
            if seconds != None:
                counter = 0
                try:
                    ...
                    if secondint < 0:
                        await ctx.send("I don't think I'm allowed to do negatives \U0001f914")
                        return
